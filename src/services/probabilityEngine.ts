 ...
 */
const getBaseDropRates = async (
    wheelTierId: number
): Promise<Result<RarityWeight[]>> => {
    try {
    ...
    `,
        ...
        const weights: RarityWeight[] = rows.map((row) => ({
 ...
 */
const applyFortuneBonus = (
    weights: RarityWeight[],
    fortuneValue: number
): RarityWeight[] => {
    if (fortuneValue <= 0) return weights;
 ...
 */
const applyMagnetismBonus = (
    weights: RarityWeight[],
    magnetismValue: number
): RarityWeight[] => {
    if (magnetismValue <= 0) return weights;
 ...
 */
const applyPurityBonus = (
    weights: RarityWeight[],
    purityValue: number
): RarityWeight[] => {
    if (purityValue <= 0) return weights;
 ...
 */
export const selectFromWeightedPool = (
    weights: RarityWeight[]
): number | null => {
 ...
 */
export const selectRarity = async (
    wheelTierId: number,
    userId: number,
    pityMode: boolean = false
 ...
 */
export const getProbabilityDistribution = async (
    wheelTierId: number,
    userId: number
): Promise<
    Result<{ rarity_name: string; probability: number; weight: number }[]>
> => {
    try {
        const weightsResult = await calculateWeights(wheelTierId, userId);
        if (!weightsResult.ok) {
