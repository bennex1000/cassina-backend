 ...
 */
export const checkPityTrigger = async (
    userId: number
 ...
 */
export const updatePityCounter = async (
    userId: number,
    rarityName: string
    ...
    try {
        ...
        if (rarityName === "Common") {
            // Increment counter
            const { rows } = await db.query<{ consecutive_common_drops: number }>(
                `
 ...
 */
export const applyPurityBonus = async (
    userId: number,
    purityValue: number
): Promise<Result<number>> => {
    try {
 ...
 */
export const getPityStatus = async (
    userId: number
): Promise<
    Result<{
        consecutive_common_drops: number;
...
> => {
    ...
    `,
        ...
        const dropsUntilPity = Math.max(
            0,
            row.pity_threshold - row.consecutive_common_drops
        );
 ...
 */
export const resetPityCounter = async (
    userId: number
): Promise<Result<boolean>> => {
    try {
        await db.query(
