 ...
 */
export const canSpin = async (userId: number): Promise<Result<boolean>> => {
    try {
        ...
        const { rows } = await db.query<UserProgression>(
            `
 ...
 */
export const getNextSpinTime = async (
    userId: number
 ...
 */
const selectRarityFromWheel = async (
    wheelTierId: number
): Promise<number | null> => {
    try {
        // Get drop rates for this wheel
        const { rows } = await db.query<WheelDropRate>(
            `
...
export const performSpin = async (
    ...
    try {
        ...
        const progressionQuery = await db.query<UserProgression>(
            `
    ...
    `,
        ...
        const rarityResult = await probabilityEngine.selectRarity(
            wheelTierId,
            userId,
            pityMode
        );
    ...
    `,
        ...
        const cooldownResult = await cooldownManager.lockCooldown(
            userId,
            speedValue
        );
