...
import { ActiveStats, SetBonus, ItemWithDetails, Result } from "../types.js";

/**
 * Calculate active stats from equipped items
 */
export const calculateActiveStats = async (
    userId: number
): Promise<Result<ActiveStats>> => {
    try {
 ...
 */
export const checkSetBonuses = async (
    userId: number
): Promise<Result<SetBonus[]>> => {
    try {
        const { rows } = await db.query<{
            stat_id: number;
            stat_name: string;
    ...
    `,
        ...
        const setBonuses: SetBonus[] = rows.map((row) => {
 ...
 */
export const getEffectiveMultipliers = async (
    userId: number
): Promise<
    Result<{
        stats: ActiveStats;
...
> => {
    try {
        ...
        const effectiveStats: ActiveStats = { ...stats };
