 ...
 */
export const addItemToInventory = async (userId, itemId) => {
    try {
        const { rows } = await db.query(`
...
export const equipItem = async (userId, itemId) => {
    try {
        ...
        const ownershipCheck = await db.query(`SELECT 1 FROM user_inventory WHERE user_id = $1 AND item_id = $2`, [userId, itemId]);
        if (ownershipCheck.rows.length === 0) {
            return { ok: false, error: "User does not own this item" };
        }
        // Get the item's slot
        const itemQuery = await db.query(`SELECT slot_id FROM items WHERE item_id = $1`, [itemId]);
 ...
 */
export const unequipItem = async (userId, slotId) => {
    try {
